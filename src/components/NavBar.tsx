...
export const NavBar = () => {
  ...
  const shuffle = () => {
    const trackList = [main, pcity, roadtovc];
    const track = trackList[Math.floor(Math.random() * trackList.length)];
    return track;
  };

  const [trackPlaying] = useState(shuffle());
  const playPause = () => {
  ...
  return (
    <div className=" flex h-20 w-full items-center justify-center bg-[#3D7DCA] shadow-md shadow-black">
      <button
        className="z-[9999] mt-1.5 mr-10 flex h-5 w-16 items-center justify-center rounded-lg bg-stone-400 pr-1 text-center"
        onClick={() => playPause()}
      >
      ...
      {/* <button
        ...
        onClick={() => skip()}
      /> */}
      <div className=" mr-20 flex h-full items-center">
